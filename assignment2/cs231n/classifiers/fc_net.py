...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a1, fcrelu1_cache = affine_relu_forward(X, W1, b1)
        ...
        da1, dW2, db2 = affine_backward(dout, fc2_cache)
        dX, dW1, db1 = affine_relu_backward(da1, fcrelu1_cache)
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        if self.num_layers == 1:
            raise ValueError(
                "FullyConnectedNet: len('hidden_dims') must be greater than one")
        ...
        if self.num_layers > 2:
            for layer in range(self.num_layers - 2):
                self.params['W%d' % (layer + 2)] = weight_scale \
                                                   * np.random.randn(
                    hidden_dims[layer],
                    hidden_dims[layer + 1])
                self.params['b%d' % (layer + 2)] = np.zeros(
                    hidden_dims[layer + 1])
                if use_batchnorm == True:
                    self.params['beta%d' % (layer + 2)] = np.zeros(
                        hidden_dims[layer + 1])
                    self.params['gamma%d' % (layer + 2)] = np.ones(
                        hidden_dims[layer + 1])
    ...
    def loss(self, X, y=None):
        ...
        for weight_num in range(self.num_layers - 1):
            W = self.params['W%d' % (weight_num + 1)]
            b = self.params['b%d' % (weight_num + 1)]
            if self.use_batchnorm == False:
                a, cache = affine_relu_forward(a, W, b)
            else:
                gamma = self.params['gamma%d' % (weight_num + 1)]
                beta = self.params['beta%d' % (weight_num + 1)]
                a, cache = affine_batchnorm_relu_forward(a, W, b, gamma, beta, self.bn_params[weight_num])
            if self.use_dropout == True:
                a, dropout_cache = dropout_forward(a, self.dropout_param)
                dropout_cache_hist.append(dropout_cache)
        ...
        loss += 0.5 * self.reg * weights_squared_sum
        ...
        # {affine - [batch norm] - relu - [dropout]} x (L - 1)
        for cache_num in range(self.num_layers - 2, -1, -1):
            if self.use_dropout == True:
                da = dropout_backward(da, dropout_cache_hist[cache_num])
            if self.use_batchnorm == False:
                da, dw, db = affine_relu_backward(da, cache_hist[cache_num])
                grads['W%d' % (cache_num + 1)] = dw
                grads['b%d' % (cache_num + 1)] = db
            else:
                da, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(da, cache_hist[cache_num])
                grads['W%d' % (cache_num + 1)] = dw
                grads['b%d' % (cache_num + 1)] = db
                grads['gamma%d' % (cache_num + 1)] = dgamma
                grads['beta%d' % (cache_num + 1)] = dbeta
