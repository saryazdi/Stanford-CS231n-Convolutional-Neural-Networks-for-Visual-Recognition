...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Backward Pass
        dh, dW_vocab, db_vocab = temporal_affine_backward(dx, cache_temporal_affine)
        if self.cell_type is 'rnn':
    ...
    def sample(self, features, max_length=30):
        ...
        x = np.ones((N,), dtype=int) * self._start
        h = h0
        c = np.zeros_like(h0)
        for timeseries in range(max_length):
            x_embedded, _ = word_embedding_forward(x, W_embed)
            if self.cell_type is 'rnn':
                h, _ = rnn_step_forward(x_embedded, h, Wx, Wh, b)
            elif self.cell_type is 'lstm':
                h, c, _ = lstm_step_forward(x_embedded, h, c, Wx, Wh, b)
            else:
                raise ValueError('Invalid cell_type "%s"' % cell_type)
            x, _ = affine_forward(h, W_vocab, b_vocab)
            x = np.argmax(x, axis=1)
            captions[:, timeseries] = x
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
